...
const filteredFracturesAndSprains = computed(() => {
  return fracturesAndSprains.filter((item) =>
    item.title.toLowerCase().includes(searchTerm.value.toLowerCase()),
  );
});

const filteredForeignBodyTitles = computed(() => {
  return foreignBodies.filter((item) =>
    item.title.toLowerCase().includes(searchTerm.value.toLowerCase()),
  );
});

const filteredWounds = computed((item) => {
  return wounds.filter((item) =>
    item.title.toLowerCase().includes(searchTerm.value.toLowerCase()),
  );
});

const filteredApathy = computed((item) => {
  return apathy.filter((item) =>
    item.title.toLowerCase().includes(searchTerm.value.toLowerCase()),
  );
});
</script>
  ...
  >
    ...
    <div>
      ...
      <v-expansion-panels class="pt-3">
        <v-expansion-panel
          v-for="item in filteredForeignBodyTitles"
          :key="item.id"
          :title="item.title"
          :text="item.text"
          class="font-semibold"
